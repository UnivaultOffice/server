...
/**
 * Preview page component with UNIVAULTOFFICE Document Editor
...
function Preview(props) {
  ...
  /**
   * Initialize the UNIVAULTOFFICE editor
   */
  ...
  useEffect(() => {
    // Load UNIVAULTOFFICE API script
  ...
  useEffect(() => {
    if (editorConfig && window.DocsAPI && editorRef.current) {
      try {
        window.docEditor = new window.DocsAPI.DocEditor('univaultoffice-editor', editorConfig);
      } catch (error) {
  ...
  return (
    <div style={{height: '100%', margin: 0}}>
      <div id='univaultoffice-editor' ref={editorRef} style={{height: '100%', width: '100%'}} />
    </div>
  );
}
